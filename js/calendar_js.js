...
function CalendarJS() {
  ...
  (this.init = function (id, initDate) {
    ...
    this.monthCell.appendChild(
      document.createTextNode(this.monthname[this.mm] + "\u00a0" + this.yy)
    );
    ...
    (this.checkDate = function () {
      ...
      window.setTimeout(function () {
        ...
        new Date(this.year, this.month, this.date, 24, 0, 0) - this.now
      ));
    }),
    ...
    (this.show = function () {
      ...
      this.monthCell.firstChild.replaceData(
        0,
  ...
  (this.createTableHead = function () {
    ...
    this.monthCell.onclick = function () {
      this.Instanz.switchMonth("current");
    };
    this.monthCell.title = this.tooltip[2];
    try {
      this.monthCell.style.cursor = "pointer";
    } catch (e) {
      this.monthCell.style.cursor = "hand";
    }
    ...
    th.Instanz = this;
    th.onclick = function () {
      this.Instanz.switchMonth("prev_year");
    };
    ...
    th.Instanz = this;
    th.onclick = function () {
      this.Instanz.switchMonth("next_year");
    };
    ...
    (this.createTableBody = function () {
      ...
      for (var i = 0; i < firstday; i++, sevendaysaweek++) {
        ...
        tr.appendChild(
          this.getCell(
            "td",
            dayspermonth[prevMonth] - firstday + i + 1,
            "last_month"
          )
        );
      }
